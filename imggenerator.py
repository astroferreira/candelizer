...
class RGBImage:
    ...
    def __init__(self, source, frames=None):
        ...
        # FITS file
            ...
            try: data = None# pyfits.getdata(arch.openbinary(os.path.expanduser(source))).T
    ...
    def applyasinh(self):
        ...
        if self.rangearr[0] <= 0: raise ValueError("Sqrt can't be applied to negative or zero pixel values")
        # apply logarithm to array values and range
        self.darr = np.arcsinh(self.darr)
        self.rangearr = ( np.arcsinh(self.rangearr[0]), np.arcsinh(self.rangearr[1]) )
...
def fix_size(img, max_size):   
    if img.shape[0] < max_size:
        imgflux = img.sum()
        img = zoom(img, max_size/img.shape[0])
        img /= img.sum()
        img *= imgflux
...
def simpletic_zoom(data, ratio):
    
    flux = data.sum()
    data = zoom(data, ratio)
    data /= data.sum()
    data *= flux
...
def plot_galaxy(name, orientation, log=True, vmin=50, ax=None, uniform_norm=True, folder='CLEAN'):
    
    if ax is None:
        f, ax = plt.subplots(1, 1, dpi=200)
    

    R = fits.getdata(f'/data/captain/MERGERS/SKIRT/TNG50-1/{folder}/F160W/{name}_{orientation}.fits')
    G = fits.getdata(f'/data/captain/MERGERS/SKIRT/TNG50-1/{folder}/F125W/{name}_{orientation}.fits')
    B = fits.getdata(f'/data/captain/MERGERS/SKIRT/TNG50-1/{folder}/F814W/{name}_{orientation}.fits')
    

    
    R = simpletic_zoom(R, B.shape[0]/R.shape[0])
    G = simpletic_zoom(G, B.shape[0]/G.shape[0])
    ...
    for vmin in range(70):
        img = np.dstack([R,G,B])
        im = RGBImage(img)
        ranges = im.percentilepixelrange(vmin, 100)
        ...
        im.scalevalues(0., 1.)
        im.applycurve()
        ratio = len(im.darr.ravel()[im.darr.ravel()>0.5])/len(im.darr.ravel()[im.darr.ravel()<0.5])
    ...
    ori_text_x = np.percentile(np.arange(shape[0]), 10)
    ori_text_y = np.percentile(np.arange(shape[1]), 10)
    
    ax.text(ori_text_x, ori_text_y, orientation, color='white', fontsize=5)
    
    from skimage.restoration import (denoise_tv_chambolle, denoise_bilateral,
                                 denoise_wavelet, estimate_sigma)
    
    
    ax.imshow(im.darr, aspect='equal', interpolation='bicubic', origin='lower')
    ax.axis('off')
    
    return ax, im.darr
    
ori = ['xy2', 'xz', 'yz', 'oct']
def plot_all_orientations(name,  log=True, vmin=50, axs=None):
    
    if axs is None:
        f, axs = plt.subplots(1, 4, dpi=200)
        axs = axs.flat
    
    for o, axi in zip(ori, axs):
        plot_galaxy(name, o,  log, vmin, ax=axi)
    
import matplotlib.pyplot as plt
plt.rcParams.update({'font.size': 5})

def plot_SED(name, ax=None):
    
    if ax is None:
        f, ax = plt.subplots(1, 1, dpi=200)
    
    for orientations in ori:
        l, w = np.loadtxt(f'/data/captain/MERGERS/SKIRT/TNG50-1/PMxSF/SED/{name}_{orientations}_sed.dat').T
        plt.plot(l, w, label=orientations, lw=1)
...
def plot_together(name,  log=True, vmin=50,):
    ...
    spec = f.add_gridspec(ncols=4, nrows=2, width_ratios=widths, height_ratios=heights)
    
    img_axs = []
    for i in range(4):
        img_axs.append(f.add_subplot(spec[0, i]))    
    
    sed_ax = f.add_subplot(spec[1, :])
    
    plot_SED(name, ax=sed_ax)
    
    plot_all_orientations(name,  log, vmin, axs=img_axs)
